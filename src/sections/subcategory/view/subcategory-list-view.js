...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.subcategory.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleSub(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.subcategory.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.subcategory.new}
              variant="contained"
