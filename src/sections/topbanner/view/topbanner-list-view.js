...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.topbanner.list);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.topbanner.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Top banner', href: paths.dashboard.topbanner.list },
            { name: 'List' },
          ]}
          action={
            <Button
