...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.foodtype.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleFoodType(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.foodtype.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Food Type', href: paths.dashboard.foodtype.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.foodtype.new}
              variant="contained"
