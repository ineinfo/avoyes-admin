...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await FetchChallenges(token);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleChallenge(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.challenges.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Challanges', href: paths.dashboard.challenges.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.challenges.new}
              variant="contained"
