...
import UserTableRow from '../faq-table-row';
import UserTableToolbar from '../faq-table-toolbar';
import UserTableFiltersResult from '../faq-table-filters-result';
import { useAuthContext } from 'src/auth/hooks';
import { DeleteFAQ, DeleteMultipleFAQ } from 'src/api/faq';
...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.faq.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleFAQ(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.faq.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'FAQ', href: paths.dashboard.faq.list },
            { name: 'List' },
          ]}
          action={
            <Button
