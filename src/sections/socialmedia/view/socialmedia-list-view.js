...
import { DeleteSocialMedia, DeleteMultipleLinks } from 'src/api/socialmedia';
...
const TABLE_HEAD = [
  ...
  { id: 'social_link', label: 'Social links' },
  { id: '', width: 88 },
];
...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.socialmedia.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleLinks(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.socialmedia.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Social links', href: paths.dashboard.socialmedia.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.socialmedia.new}
              variant="contained"
