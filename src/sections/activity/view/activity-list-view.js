...
const TABLE_HEAD = [
  ...
  { id: 'start_datetime', label: 'Start Date' },
  { id: 'end_datetime', label: 'End Date' },
  { id: 'location', label: 'Location' },
  { id: 'hosted_by', label: 'Hosted By' },
  { id: '', width: 88 },
];
...
export default function UserListView() {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleActivity(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.activity.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Activity', href: paths.dashboard.activity.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.activity.new}
              variant="contained"
