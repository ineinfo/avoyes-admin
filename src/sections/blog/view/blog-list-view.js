...
const TABLE_HEAD = [
  ...
  { id: 'blog_category_title', label: 'Blog Category' },
  { id: '', width: 88 },
];
...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.blog.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleblog(selectedIds, token);
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Blog', href: paths.dashboard.blog.list },
            { name: 'List' },
          ]}
          action={
            <Button
