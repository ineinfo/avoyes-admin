...
export default function UserListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.populardishes.list);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await DeletePopularDishes(id, token);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultiplePopularDishes(selectedIds, token);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.populardishes.edit(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Popular Dishes', href: paths.dashboard.populardishes.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.populardishes.new}
              variant="contained"
